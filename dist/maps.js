...
function initialize() {
 ...
 var userRegister = function(name,email,pwd)
  ...
  var loadNavigations =  function(){
    ...
    window.mapbuttonclick = function(targetelement, plusorminus, inverseelement) {
          ...
          for(subcat in data[cat])
          {
            for (latlng in data[cat][subcat])
            {
              ...
              { 
                ...
                rowString += "<\/td><td><button onclick = \"window.mapbuttonclick('map-button-"+i+"-"+1+"',1, 'map-button-"+i+"-"+2+"');\" id = \"map-button-"+i+"-"+1+"\" style = \"border-radius: 3px;margin-left: 10px;cursor:pointer; color: white;background: #f44336 ; border: none; ;float:right; width: 40px; height: 20px;\">+" + latLngArray[i][1] + "<\/button>";
                rowString += "<button onclick = \"window.mapbuttonclick('map-button-"+i+"-"+2+"',2,'map-button-"+i+"-"+1+"');\" id = \"map-button-"+i+"-"+2+"\" style = \"border-radius: 3px;margin-left: 10px;cursor:pointer; color: white;background: #f44336 ; border: none; ;float:right; width: 40px; height: 20px;\">-" + latLngArray[i][2] + "<\/button>";
                rowString += "<\/td>";
                rowString += "<\/tr>";          
              }  
